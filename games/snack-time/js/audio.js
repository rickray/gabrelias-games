/* Snack Time — unlock, speak, yum, bounce. */
...
(function (global) {
  ...
  function beep(freq, dur, type, peak, slide) {
    if (!ensure()) return;
    var t = t0();
    ...
    var g = env(t, peak || 0.22, 0.01, 0.02, dur);
    o.connect(g);
    g.connect(master);
    o.start(t);
    o.stop(t + dur + 0.05);
  ...
  function yum() {
    ...
    for (i = 0; i < notes.length; i++) {
      (function (freq, when) {
        ...
        var g = env(when, 0.2, 0.012, 0.04, 0.16);
        o.connect(g);
        g.connect(master);
        o.start(when);
        o.stop(when + 0.22);
  ...
  function bounce() {
    if (!ensure()) return;
    beep(240, 0.16, "triangle", 0.18, 140);
  }
